...
def dijkstra(line_Info, StartPoint, INF=float('inf')):
    ...
    while len(book) < len(line_Info):
        book.add(min_V)  # 确定输入起点（向book集合中加入新元素min_V）
        # print(book) （调试用）
...
# while(True):
#     if flag > 0:
#         accept = input('是否继续计算？(y/n)：')
#         if accept == 'n' :
#             break
#         elif accept == 'y':
#             startpoint = input('输入起点：')
#             try:
#                 disAll = dijkstra(Line_Info, StartPoint=startpoint)
...
#             needRmd = 6
#         if distance / 1000 > 32:
#             needRmd = 7 + (distance / 1000 - 32) // 20  # 整数除法
#         print('票价为' + str(needRmd) + '元 ')
